...
Binaris_fa::Binaris_fa(Point xy, int szint, string el_tipus) : szintek(szint)
{
	...
	for(int i=2; i<=szint; ++i) //Pl. 3 szint
		for(int j=0; j<pow(2,i-1); ++j)  //2^1,2^2 --> (2,4) 
		{
		//std::cout << "Check --> " << pow(2,i-1) << std::endl; 
...
void Binaris_fa_kocka::draw_lines() const
{
	...
	{
		fl_color(fill_color().as_int());
		for (int i = 0; i < number_of_points(); i++) //Kocka kirajzolása
		{ 
			fl_rectf(point(i).x-k, point(i).y-k, k+k, k+k);
		}
		fl_color(color().as_int());
...
void Binaris_fa::cimkek(string cimke1, string cimke2)
{
	...
	int csomopont_elem = 0;
	while (iss.get(ch))
	{
		switch(ch) //Pl. 3 szint, és 1(gy) | 2(b-j) | 4(bjbj)  --> gy | 1.b 2.j | 3.b 4.j 5.b 6.j |
		{ //Ha "bbb" --> 0-1, 1-3 
		case 'b': 
			csomopont_elem = 2*csomopont_elem+1;
			//std::cout << "Check: " << csomopont_elem << std::endl;
			break;
		case 'j':
			csomopont_elem = 2*csomopont_elem+2;
			//std::cout << "Check: " << csomopont_elem << std::endl;
			break;
		}
	}
	cimkek_v[csomopont_elem].set_label(cimke2);
}
