...
protected:
	...
	void pontok_torlese() { points.clear();}
...
struct Nyil : Shape
{
	Nyil(Point kezdo, Point vege, double meret) : nyilhegy_meret(meret)
	{
		add(kezdo);
		add(vege);
	} 
...
struct SzovegBox : Text
{
	...
	//Eltoljuk 15px x 20px --> Dobox (xy koord., szöveg hosszúság * betűmérettel, font_size + 15 magas, radius)
...
{
	Smiley(Point p, int radius) : Circle(p,radius) {}
	void draw_lines() const;
};

struct Frowny : Circle
{
	Frowny(Point p, int radius) : Circle(p,radius) {}
	void draw_lines() const;
};
...
struct Exercises_function : Function 
{
	Exercises_function(Fct f, double r1, double r2, Point orig, int count = 100, double xscale = 25, double yscale = 25, double precision = 1);
	void reset_fct(Fct f) { fct = f; reset(); }
	void reset_range(double r1, double r2);
	void reset_origo(Point orig) {origin = orig; reset();}
	void reset_count(int count);
	void reset_xskala(double xskala);
	void reset_yskala(double yskala);
...
struct Binaris_fa : Shape
{
	Binaris_fa(Point xy, int szint , string el_tipus ); //el_tipus: "sima, nyil"
	
	virtual void draw_lines() const;
	int szint() const { return szintek; }
	void cimkek(string cimke1,string cimke2);
	
	protected:
		Vector_ref<Shape> elek;
		Vector_ref<Text> cimkek_v;
	private:
		int szintek;
};

struct Binaris_fa_kocka : Binaris_fa
{
	Binaris_fa_kocka(Point xy, int szint , string el_tipus ) : Binaris_fa(xy,szint,el_tipus) {}
	
	void draw_lines() const;
};

struct Haromszog : Polygon
{
	Haromszog(Point p1, Point p2, Point p3)
	{
		Polygon::add(p1);
		Polygon::add(p2);
		Polygon::add(p3);
...
struct Binaris_fa_haromszog : Binaris_fa
{
	Binaris_fa_haromszog(Point xy, int szint , string el_tipus ) : Binaris_fa(xy,szint,el_tipus) {}
